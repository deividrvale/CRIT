...
case class Equation(left: Term, right : Term, constraint : Constraint) {
  ...
  def replaceSide(side: Side, term: Term): Equation = side match {
    case Side.Left => Equation(term, right, constraint)
    case Side.Right => Equation(left, term, constraint)
  }

  def applyRuleToSide(side: Side, f: Term => Term): Equation = side match {
    case Side.Left => Equation(f(left), right, constraint)
    case Side.Right => Equation(left, f(right), constraint)
  }
